...
description: Generate beautiful, self-contained HTML pages that visually explain systems, code changes, plans, and data. Use when the user asks for a diagram, architecture overview, plan review, comparison table, or any visual explanation of technical concepts. Also use proactively when about to render a complex ASCII table (4+ rows or 3+ columns).
---
...
**Output location:** Write to `.claude/tasks/views/` (relative to the repo root). Use a descriptive filename based on content: `plan-review-auth-system.html`, `pipeline-flow.html`, `schema-overview.html`. The directory persists across sessions and is co-located with task files.

**Open in browser (cross-platform):**

- Windows: `start .claude/tasks/views/filename.html` (in Git Bash) or `start "" ".claude\tasks\views\filename.html"` (in cmd)
- macOS: `open .claude/tasks/views/filename.html`
- Linux: `xdg-open .claude/tasks/views/filename.html`

Detect the platform and use the appropriate command. On Windows with Git Bash, `start` works with forward slashes.
...
**Use Mermaid.** Data flow diagrams emphasize connections over boxes. Use `graph LR` or `graph TD` with edge labels for data descriptions. Thicker, colored edges for primary flows. Source/sink nodes styled differently from transform nodes via Mermaid's `classDef`.
...
Use a real `<table>` element. Tables get accessibility, copy-paste behavior, and column alignment for free. The reference template at `./templates/data-table.html` demonstrates all patterns.
...
- **Mermaid zoom controls**: Every `.mermaid-wrap` container must have zoom controls (+/-/reset/expand buttons), Ctrl/Cmd+scroll zoom, click-and-drag panning, and a fullscreen expand button that opens the diagram in a fixed overlay. Complex diagrams render too small without them. The cursor should change to `grab` when zoomed in and `grabbing` while dragging. The fullscreen overlay must include its own zoom controls and close on Escape. See `./references/css-patterns.md` for the full pattern.
