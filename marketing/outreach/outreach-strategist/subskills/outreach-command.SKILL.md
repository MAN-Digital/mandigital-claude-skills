...
Runs the real outreach pipeline end-to-end and lands the result in Mission
Control (MC) for human review. Every step reads/writes the ONE campaign
envelope (guard §8); the LLM steps are the sibling skills invoked headlessly
per guard §10. **Nothing sends. Ever.** Approval produces DRAFTS only.

## Invocation

```
/outreach <offer> <hubspot-list-or-segment> [--channel woodpecker|both|auto]
```

The same pipeline also runs automatically when MC's "+ New campaign" intake
fires the watcher (`scripts/signal_processor.py` in this skill's `scripts/`).

---

## The pipeline (each step = one envelope pass)

```
contacts → signals → cadence-strategist → copywriter → copy-qa → MC upsert
                                                                    │
                                              MC gates (human) ─────┤
                                                                    ▼
                                     build approve → stage DRAFTS (Woodpecker
                                     DRAFT campaigns: email + LinkedIn steps) — only
```

### Step 0 — Contacts

Resolve the audience: pull the HubSpot list/segment named in the intake
(HubSpot is the CRM source of truth — see BD hand-off below), enrich to
`contacts_json.contacts` (name, title, company, email, linkedin, segment,
prior-contact history). Dev/test runs use `.example` contacts only.

### Step 1 — Signals (deterministic, engine)

`signals_source` (engine, config-driven by the campaign's Connections) queries
the wired providers — Exa (news/funding/launches), Apollo (firmographics,
job changes, hiring), HarvestAPI (LinkedIn posts/activity) — and writes
`strategy_json.signals` per contract **C1** (guard §10): per-contact and
per-account signals with `trigger_code` (from the vocabulary), `url`, `date`,
`provider`, plus an honest `no_signal` list. **Empty results are recorded
honestly — never fabricated.** Research deepening also lands
`strategy_json.research_inbox` (proof-first, user-written sources with URLs)
and `strategy_json.voc_bank`.

### Steps 2–4 — The LLM skills, invoked headlessly (contract C6)

Per step, the engine writes the whole envelope to the run dir, then invokes the
skill in the openclaw agent runtime with a message naming the skill and the
envelope path. The skill writes ONLY its artifact next to the envelope and
prints its DONE marker; the engine validates (schema, non-emptiness, framework
ids ∈ vocabulary) and merges. A missing/invalid artifact is a HARD FAIL —
honest exit, never a silent shim.

| Step | Skill                         | Reads (highlights)                                                                                                                | Writes                                                                                                                                                                                                               | Marker         |
| ---- | ----------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| 2    | `outreach-cadence-strategist` | settings_json.campaign (C5), messaging_vocabulary, positioning_docs, signals (C1), research + voc_bank, learnings chips, contacts | `cadence.json` (C2: ONE funnel stage, ≥5 touches over ~12 days, email+LinkedIn, framework/angle ids per touch, honest signal_ref, per-touch `basis`, ENROLL/HOLD/EXCLUDE)                                            | `DONE_CADENCE` |
| 3    | `outreach-copywriter`         | the assigned touches + C1 signals + use case + proof sources                                                                      | `copy.json` (C3: by_segment AND by_contact; per-contact opener on that contact's REAL signal w/ url, or honest cold fallback; every claim traced in `claims`)                                                        | `DONE_COPY`    |
| 4    | `outreach-copy-qa`            | the copy + assignments + C1 + proof + copy rules                                                                                  | `qa.json` (C4: deterministic linter checks + judged dimensions signal_present / framework_honored / personalization_specific / no_fabricated_stats / …; honest hard_failures; blocked touches ⇒ disposition blocked) | `DONE_QA`      |

QA hard failures loop back: fix via the copywriter, re-run QA. A blocked touch
never advances.

### Step 5 — MC upsert + human gates

The engine composes `settings_json.campaign` (C5 — the applied per-campaign
config: ONE funnel stage, ONE use case, actual channels/connections) and
upserts the full envelope into Mission Control. Humans review there:
**strategy gate → build gate → presend confirm.** Gate feedback lands in
`learnings_json` and MUST alter the next build (the strategist cites chips in
`basis`).

### Step 6 — Staging on BUILD APPROVAL (drafts only — the invariant)

When Romeo approves the build gate in MC, the MC backend stages:

- **Woodpecker**: `POST /rest/v2/campaigns` → always a **DRAFT** campaign
  (email touches; LinkedIn steps only if a Woodpecker linkedin_account exists) +
  prospects added. The run endpoint is NEVER called.
- **LinkedIn**: LinkedIn touches become **LINKEDIN steps** (connection
  request / DM) inside the same Woodpecker DRAFT — requires a connected
  LinkedIn seat in Woodpecker; otherwise the draft is email-only + an
  honest note (SendPilot was removed 2026-07-02).
  called. No draft campaign configured ⇒ honest
  `needs_manual_campaign_shell` — never faked.

Real returned ids/links go to `links_json`. Presend confirm is a human marker
only — it also sends nothing. **DRAFT-ONLY INVARIANT: no code path in this
pipeline may call any Woodpecker start/run/send endpoint.**

---

## Run artifacts

**Run directory:** `$OUTREACH_RUNS_DIR/<campaign-slug>/` (env-driven; the
container default is `skills/business-development/outreach-runs/<slug>/`).
`<campaign-slug>` per `business-development/references/campaign-naming.md`
(`<STAGE>-<GOAL>-<INDUSTRY[.SUB]>-<SEGMENT>-<GEO>-<YYMM>`).

| File            | Written by         | Contents                                                |
| --------------- | ------------------ | ------------------------------------------------------- |
| `envelope.json` | engine             | the WHOLE campaign envelope (input to every skill step) |
| `cadence.json`  | cadence-strategist | C2 sequence + fit grades                                |
| `copy.json`     | copywriter         | C3 by_segment + by_contact copy                         |
| `qa.json`       | copy-qa            | C4 multi-dimensional verdict                            |
| `review.md`     | pipeline           | human-readable summary                                  |

---

## BD / EA knowledge hand-off (how their context enters the envelope)

The pipeline does not re-invent company knowledge — it consumes what the
`business-development` and `executive-assistant` skills already maintain.

**From `business-development`** (see `skills/business-development/SKILL.md`):

- **Positioning / ICP / personas** — BD's source of truth is
  `references/MAN Digital Positioning/` (`00-README.md` index:
  `positioning-canvas.md`, `08-icp.md`, `09-persona-messaging-guide.md`,
  `05-use-case-messaging.md`, `02-anti-value-proposition.md`,
  `persona-job-titles.md`). The SAME library is what the MC profile's
  `config/positioning/` docs mirror — the engine loads it into
  `settings_json.positioning_docs` (via `positioning.py` +
  `OUTREACH_PROFILE_PATH`), which is how the strategist/copywriter read it.
- **Audience + CRM state** — HubSpot is BD's CRM source of truth. Step 0 pulls
  the HubSpot list; per-contact history (prior blasts, `hs_lead_status` like
  BAD_TIMING, lifecycle) comes via BD's
  `scripts/hubspot_account_brief.py` pattern and lands on the contact record in
  `contacts_json` so a known contact is never written as cold.
- **Warm paths** — BD's `scripts/warm_network.py` (LeadDelta + HubSpot warmth
  tiers) informs fit grading: a HOT/WARM contact is noted in the contact's
  `reason`/history for the strategist.
- **Outreach tracker** — BD owns the live tracker + reply handling
  (`scripts/woodpecker_reply_notify.py` cron). Campaign outcomes flow back
  through BD, not this pipeline.
- **Email quality contract** — BD's `references/outreach-copywriting.md` is the
  historical voice contract; the operational rules now live in the copywriter
  skill + `settings_json.copy_rules`.

**From `executive-assistant`** (see `skills/executive-assistant/SKILL.md`):

- **Client/relationship knowledge** — the EA maintains meeting/relationship
  state: `clawchief/meeting-notes.md` (Granola ingestion policy),
  `memory/meeting-notes-state.json`, discovery-call state in
  `.ea_state/<meeting_id>.json`, tasks in `clawchief/tasks.md`, and the
  ontology (`skills/ontology`). When a campaign audience overlaps people Romeo
  has actually met or is in conversation with, that knowledge MUST reach the
  envelope: flag such contacts in `contacts_json` (history/re-engagement) so
  the strategist HOLDs or re-frames them — never cold-blast an active
  relationship.
- **Meeting links policy** — `clawchief/meeting-links.md` (HubSpot links for
  sales, Reclaim for non-sales, never in a first cold touch) is the source of
  the booking-link rules the copywriter and QA enforce via
  `settings_json.campaign.booking_link`.
- **Surfacing** — replies/interest reach Romeo through the EA thread (the
  Woodpecker reply cron posts there); this pipeline itself never messages
  anyone.

Honest current state: positioning docs and HubSpot lists are wired into the
envelope automatically; warm-network and EA meeting-state checks are performed
by THIS skill when preparing contacts (run the BD scripts above) — they are not
yet an automatic engine step. Do them; don't claim the engine did.
...
| `channel_mode` | Effect                                                       |
| -------------- | ------------------------------------------------------------ |
| `woodpecker`   | email-only touches; Woodpecker DRAFT on approval             |
| `sendpilot`    | LEGACY value — treated as LinkedIn-lean; stages Woodpecker   |
| `both`         | coordinated multichannel, never same-day per person          |
| `auto`         | strategist decides from audience LinkedIn activity + signals |

One live conversation per person across channels; a reply anywhere stops the
other channel.
...
- **Nothing sends without explicit human action outside this pipeline.**
  Build approval creates DRAFTS/leads only; presend confirm is a marker.
  No start/run/send endpoint, ever.
- A QA hard-block never reaches staging — loop back to the copywriter.
- The fit gate is outreach-only; CRM population is unaffected.
- **Never fabricate a signal, stat, quote, customer, or relationship** — C1 is
  the only signal source; the honest cold fallback covers the rest.
- Config-driven everything: vocabulary/frameworks from
  `settings_json.messaging_vocabulary` (§9), paths from env
  (`$OUTREACH_RUNS_DIR`, `OUTREACH_PROFILE_PATH`), providers from Connections.
- A shim/fixture result must never be presented as a real skill output; a
  missing artifact is a hard failure, reported as one (§10).
