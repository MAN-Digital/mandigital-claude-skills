...
when_to_use: LAST stage of the pipeline — after the cut is locked, pause pass and speedup executed, and the DaVinci caption template applied. Both exports must come from the SAME conformed cut. Also re-triggered whenever a fresh SRT export arrives after timeline changes. Never mid-edit — any recut voids every timecode here.
