...
9. **Use the canonical website components.** Team, form, FAQ, proof and decoration have
   ...
   name/company/role fields, and two results grounded in approved evidence. Use `RESULTS` as
   the visible proof-row label; never show `VERIFIED RESULTS` to the customer. Treat
   verification as an internal content gate. Prefer one attributable approved client quote.
   When no approved quote exists, use a clearly labeled outcome sourced from the brief
